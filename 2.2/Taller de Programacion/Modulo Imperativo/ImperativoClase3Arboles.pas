...
begin
  ...
  then writeln ('Arbol sin elementos')
         ...
         writeln;
         writeln ('Numero de socio con mas edad: ', maxNum, ' (',maxEdad ,')');
         writeln;
       end;
end;
...
procedure InformarCantidadSocios (a:arbol);
	procedure contarSocios(a:arbol; var c:integer);
	begin
		if(a <> nil)then begin
			contarSocios(a^.HI,c);
			contarSocios(a^.HD,c);
...
begin
	cant:=0;
	contarSocios(a,cant);
	Writeln('Se encontraron ',cant,' socios');
end;
