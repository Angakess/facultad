...
procedure opcionB(var empleados: archivoEmpleados);
	...
	procedure opcionVIII(var empleados:archivoEmpleados);
	var
		ult, aux:empleado;
		numX, posAct: integer;
	begin
		write('Ingrese el numero de empleado que quiera borrar: ');
		readln(numX);
		read(empleados, aux);
		while((numX <> aux.num)and(not eof(empleados)))do begin
			read(empleados, aux);
		end;
		if(numX = aux.num)then begin
			posAct := (filepos(empleados))-1;
			seek(empleados, (filesize(empleados)-1));
			read(empleados, ult);
			seek(empleados, posAct);
			write(empleados, ult);
		end
		else
			writeln('No se encontro el empleado con ese numero');
		seek(empleados, (filesize(empleados)-1));
	end;
var
	opcion:integer;
begin
	...
	Writeln('8 - Borrar un empleado. ');
	readln(opcion);
	while((opcion <> 1)and(opcion <> 2)and(opcion <> 3)and(opcion <> 4)and(opcion <> 5)and(opcion <> 6)and(opcion <> 7)and(opcion <> 8))do begin
